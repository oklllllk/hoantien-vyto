...
// API trả về (shopeeComFinal / price) LỚN HƠN 0% thì tính lại theo mức mới
// 8%, cap tối đa 50.000đ. Cộng thêm hoa hồng Xtra (sellerComFinal, không cap,
// nhưng nếu âm thì coi như 0) để ra tổng hoa hồng thực nhận.
...
function calcCommission(p) {
  ...
  const sellerComRaw = Number(p.sellerComFinal || 0); // hoa hồng Xtra
  const sellerCom = Math.max(sellerComRaw, 0); // Xtra âm → coi như 0, không trừ vào tổng
  const shopeeComRaw = Number(p.shopeeComFinal || 0); // hoa hồng Shopee do API trả về

  // Ép lên 8% nếu Shopee trả về LỚN HƠN 0% (chỉ cần > 0%, không cần ngưỡng nào khác)
  let shopeeCom = shopeeComRaw;
  const isOldScheme = price > 0 && shopeeComRaw / price > 0;
  if (isOldScheme) {
