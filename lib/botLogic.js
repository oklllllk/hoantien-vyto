 ...
 *   hoan_thanh_chua_rut = tổng (hoa_hong * 90% * 80%) của đơn hoàn thành < 3 ngày
...
export function calcVitien(vitienData, daNhanData, subId) {
  ...
  for (const don of v.don_hoan_thanh || []) {
    ...
    if (soNgay >= 3) {
      // Đã đủ 3 ngày trở lên: chuyển hẳn sang "Có sẵn để rút", không còn
      // tính vào "Đã hoàn thành" nữa.
      coTheRut = Math.round((coTheRut + netAmount) * 100) / 100;
    } else {
      // Chưa đủ 3 ngày: vẫn nằm ở "Đã hoàn thành", nhưng đã áp dụng đúng
